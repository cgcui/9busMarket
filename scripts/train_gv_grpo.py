...
def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--config", type=Path, required=True); ap.add_argument("--run-dir", type=Path, required=True); ap.add_argument("--candidates", type=Path, required=True); ap.add_argument("--gate-report", type=Path, required=True); ap.add_argument("--output", type=Path, required=True); args = ap.parse_args()
    ...
    params = [p for p in model.parameters() if p.requires_grad]; optimizer = torch.optim.AdamW(params, lr=float(cfg["learning_rate"])); groups = list(grouped.values()); random.seed(int(cfg["seed"])); history = []
    for step in range(int(cfg["optimizer_steps"])):
        ...
        with torch.no_grad(): old_lp = sequence_logprob(model, tok, prompts, outputs, int(cfg["max_seq_length"])).detach()
    ...
    report = {"protocol": "Paper9Bus-Power-GV-GRPO-v3", "stage": "GV_GRPO", "optimizer_steps": len(history), "group_size": int(cfg["group_size"]), "horizon": int(cfg["horizon"]), "gamma": float(cfg["gamma"]), "clip_eps": float(cfg["clip_eps"]), "kl_coef": float(cfg["kl_coef"]), "source_candidates": str(args.candidates), "gate_status": gate["status"], "final_accessed": False, "history": history}
